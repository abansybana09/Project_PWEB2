...
if (!preg_match("/^[0-9]{4}$/", $selected_year)) { die("Format tahun tidak valid."); }
...
$sql_orders = "SELECT id, pelanggan, nohp, pesanan, jumlah_pesan, total_harga, waktu_order_dibuat FROM tb_order WHERE DATE_FORMAT(waktu_order_dibuat, '%Y') = ? AND LOWER(pembayaran) IN ('ya', 'success', 'settlement') ORDER BY waktu_order_dibuat ASC";
...
class PDF_Tahunan extends FPDF
{
    function Header()
    {
        ...
        $this->SetFont('Arial','B',12); $this->Cell(0,10,'Laporan Omset Tahunan',0,1,'C');
        $this->SetFont('Arial','',10); $this->Cell(0,5,'Tahun: '.$selected_year,0,1,'C'); $this->Ln(10);
    }
    ...
    function FancyTable($header, $data)
    {
        ...
        $w = array(8, 22, 15, 30, 25, 40, 15, 35); // No, Tgl Lengkap, ID, Plgn, No.HP, Pesanan, Jml, Total
        ...
        $fill = false; $nomor = 1; global $total_omset_tahunan_pdf;

        if (empty($data)) { $this->Cell(array_sum($w),10,'Tidak ada transaksi untuk tahun ini.',1,1,'C'); }
        else {
            foreach($data as $row) {
                $current_y_start = $this->GetY(); $cell_height = 6;
                $this->Cell($w[0],$cell_height,$nomor++,'LR',0,'C',$fill);
                $this->Cell($w[1],$cell_height,date('d/m/Y H:i', strtotime($row['waktu_order_dibuat'])),'LR',0,'C',$fill);
                ...
                for($k=0; $k < count($w); $k++){ $this->Cell($w[$k],$row_height,'','LR',0,'C',$fill); }
        ...
        $this->Cell($w[count($w)-1], 8, 'Rp ' . number_format($total_omset_tahunan_pdf, 0, ',', '.'), 1, 1, 'R', true);
    }
}

$pdf = new PDF_Tahunan('P','mm','A4'); $pdf->AliasNbPages(); $pdf->AddPage();
$header = array('No.', 'Waktu', 'ID', 'Pelanggan', 'No. HP', 'Pesanan', 'Jml', 'Total');
