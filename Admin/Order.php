...
if (!$conn || $conn->connect_error) {
    $conn_error = "Koneksi DB gagal: " . ($conn->connect_error ?? 'Tidak diketahui');
    ...
    if ($stmt_daily) {
        $stmt_daily->bind_param("s", $filter_tanggal);
        if ($stmt_daily->execute()) {
            $result_daily = $stmt_daily->get_result();
            if ($row_daily = $result_daily->fetch_assoc()) {
                $daily_omset = (float)($row_daily['total_omset_harian'] ?? 0);
            }
        } else {
            $query_error_messages[] = "Exec daily: " . $stmt_daily->error;
        }
        $stmt_daily->close();
    } else {
        $query_error_messages[] = "Prep daily: " . $conn->error;
    }
    ...
    if ($stmt_monthly) {
        ...
        if ($stmt_monthly->execute()) {
            ...
            if ($row_monthly = $result_monthly->fetch_assoc()) {
                $monthly_omset = (float)($row_monthly['total_omset_bulanan'] ?? 0);
            }
        } else {
            $query_error_messages[] = "Exec monthly: " . $stmt_monthly->error;
        }
        $stmt_monthly->close();
    } else {
        $query_error_messages[] = "Prep monthly: " . $conn->error;
    }
    ...
    if ($stmt_yearly) {
        ...
        if ($stmt_yearly->execute()) {
            $result_yearly = $stmt_yearly->get_result();
            if ($row_yearly = $result_yearly->fetch_assoc()) {
                $yearly_omset = (float)($row_yearly['total_omset_tahunan'] ?? 0);
            }
        } else {
            $query_error_messages[] = "Exec yearly: " . $stmt_yearly->error;
        }
        $stmt_yearly->close();
    } else {
        $query_error_messages[] = "Prep yearly: " . $conn->error;
    }
    ...
    if ($stmt_orders_table) {
        ...
        if ($stmt_orders_table->execute()) {
            ...
            $query_error_messages[] = "Execute orders table: " . $stmt_orders_table->error;
        }
        $stmt_orders_table->close();
    } else {
        $query_error_messages[] = "Prepare orders table: " . $conn->error;
    }
}

if (!empty($query_error_messages)) {
    error_log("Admin/Order.php SQL Errors: " . implode(" | ", $query_error_messages));
}
?>
...
<div class="col-lg-9 mt-2">
    <div class="card">
        ...
        <div class="card-body">
            ...
            if ($conn_error) {
                echo '<div class="alert alert-danger" role="alert">' . htmlspecialchars($conn_error) . '</div>';
            }
            if (!empty($query_error_messages) && !$conn_error) {
                echo '<div class="alert alert-warning" role="alert">Masalah query: ' . htmlspecialchars(implode("; ", $query_error_messages)) . ' (Periksa log server untuk detail)</div>';
            }
            ?>
            ...
            <form method="GET" action="<?= htmlspecialchars($_SERVER['PHP_SELF']) ?>">
                <?php
                if (isset($_GET['x'])) {
                    echo '<input type="hidden" name="x" value="' . htmlspecialchars($_GET['x']) . '">';
            ...
            <?php
            if (!$conn_error && empty(array_filter($query_error_messages, function ($msg) {
                return strpos(strtolower($msg), 'orders table') !== false;
            }))) :
                if (!empty($orders_for_table)) :
            ?>
                    <div class="table-responsive">
                        <div class="d-flex justify-content-end">
                            <div class="mb-3" style="width: 200px;"><input type="text" id="searchInput" class="form-control form-control-sm" placeholder="Cari Pelanggan"></div>
                        </div>
                        ...
                        <table class="table table-hover table-sm" id="orderTable">
                            <thead>
                                <tr class="text-nowrap">
                                    ...
                                    <th scope="col" style="min-width: 200px;">Pesanan</th>
                                    <th scope="col">Jml</th>
                                ...
                                ?>
                                        ...
                                        <div class="modal-dialog modal-xl">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="ViewDataLabel<?= $row['id'] ?>">Detail Order ID: <?= htmlspecialchars($row['id']) ?></h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
                                                    <p><strong>Pelanggan:</strong> <?= htmlspecialchars($row['pelanggan']) ?></p>
                                                    <p><strong>No HP:</strong> <?= htmlspecialchars($row['nohp']) ?></p>
                                                    <p><strong>Alamat:</strong> <?= htmlspecialchars($row['alamat']) ?></p>
                                                    <p><strong>Pesanan:</strong><br>
                                                    <div style="white-space: pre-wrap; word-break: break-word;"><?= htmlspecialchars($row['pesanan']) ?></div>
                                                    </p>
                                                    <hr>
                                                    <p><strong>Jumlah:</strong> <?= htmlspecialchars($row['jumlah_pesan']) ?></p>
                                                    <p><strong>Total Harga:</strong> <?= 'Rp ' . number_format((float)preg_replace('/[^0-9.]/', '', $row['total_harga']), 0, ',', '.') ?></p>
                                                    <p><strong>Status Pembayaran:</strong> <?= htmlspecialchars($row['pembayaran']) ?></p>
                                                    <p><small>Waktu Order: <?= htmlspecialchars(date('d M Y H:i:s', strtotime($row['waktu_order_dibuat']))) ?></small></p>
                                    ...
                                    <div class="modal fade" id="EditData<?= $row['id'] ?>" tabindex="-1" aria-labelledby="EditDataLabel<?= $row['id'] ?>" aria-hidden="true">
                                        <div class="modal-dialog modal-xl">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="EditDataLabel<?= $row['id'] ?>">Edit Order ID: <?= htmlspecialchars($row['id']) ?></h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <form action="../Proses/Edit_order.php" method="POST" class="needs-validation" novalidate>
                                                    <div class="modal-body">
                                                        ...
                                                        <div class="row">
                                                            <div class="col-md-6">
                                                                <div class="form-floating mb-3"><input type="text" class="form-control" name="pelanggan" required value="<?= htmlspecialchars($row['pelanggan']) ?>"><label>Nama Pelanggan</label>
                                                                    <div class="invalid-feedback">Wajib diisi.</div>
                                                                </div>
                                                            </div>
                                                            <div class="col-md-6">
                                                                <div class="form-floating mb-3"><input type="text" class="form-control" name="nohp" required value="<?= htmlspecialchars($row['nohp']) ?>"><label>No HP</label>
                                                                    <div class="invalid-feedback">Wajib diisi.</div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="row">
                                                            <div class="col-12">
                                                                <div class="form-floating mb-3"><input type="text" class="form-control" name="alamat" required value="<?= htmlspecialchars($row['alamat']) ?>"><label>Alamat</label>
                                                                    <div class="invalid-feedback">Wajib diisi.</div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="row">
                                                            <div class="col-12">
                                                                <div class="form-floating mb-3"><textarea class="form-control" style="height: 100px" name="pesanan" required><?= htmlspecialchars($row['pesanan']) ?></textarea><label>Pesanan</label>
                                                                    <div class="invalid-feedback">Wajib diisi.</div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="row">
                                                            <div class="col-md-4">
                                                                <div class="form-floating mb-3"><input type="number" class="form-control" name="jumlah_pesan" required value="<?= htmlspecialchars($row['jumlah_pesan']) ?>" min="1"><label>Jumlah Pesanan</label>
                                                                    <div class="invalid-feedback">Wajib (angka).</div>
                                                                </div>
                                                            </div>
                                                            <div class="col-md-4">
                                                                <div class="form-floating mb-3"><input type="number" step="any" class="form-control" name="total_harga" required value="<?= preg_replace('/[^0-9.]/', '', $row['total_harga']) ?>"><label>Total Harga (Angka)</label>
                                                                    <div class="invalid-feedback">Wajib (angka).</div>
                                                                </div>
                                                            </div>
                                                            <div class="col-md-4">
                                                                <div class="form-floating mb-3"><select class="form-select" name="pembayaran" required>
                                                                    ...
                                                                    </select><label>Pembayaran</label>
                                                                    <div class="invalid-feedback">Wajib dipilih.</div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button><button type="submit" class="btn btn-primary" name="submit_edit_order">Save</button></div>
                                                </form>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="modal fade" id="HapusData<?= $row['id'] ?>" tabindex="-1" aria-labelledby="HapusDataLabel<?= $row['id'] ?>" aria-hidden="true">
                                        <div class="modal-dialog modal-md">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h1 class="modal-title fs-5" id="HapusDataLabel<?= $row['id'] ?>">Hapus Order ID: <?= htmlspecialchars($row['id']) ?></h1><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
...
<script>
    ...
    async function initiateAdminSnapPayment(orderId) {
        ...
        if (!snapMessageDiv) {
            console.error("Elemen adminSnapMessage tidak ditemukan!");
            return;
        }
        ...
        try {
            ...
            if (result.success && result.snap_token) {
                ...
                if (typeof snap === 'undefined') {
                    throw new Error("Library Midtrans Snap.js tidak siap.");
                }
                snap.pay(result.snap_token, {
                    onSuccess: function(paymentResult) {
                        console.log('Admin Payment Success:', paymentResult);
                        snapMessageDiv.innerHTML = `<div class="alert alert-success">Pembayaran Order ID ${paymentResult.order_id} berhasil! Status: ${paymentResult.transaction_status}. Reload...</div>`;
                        setTimeout(() => {
                    ...
                    onPending: function(paymentResult) {
                        ...
                        snapMessageDiv.innerHTML = `<div class="alert alert-warning">Pembayaran Order ID ${paymentResult.order_id} tertunda. Status: ${paymentResult.transaction_status}.</div>`;
                        if (clickedButton) clickedButton.disabled = false;
                    },
                    onError: function(paymentResult) {
                        console.error('Admin Payment Error:', paymentResult);
                        snapMessageDiv.innerHTML = `<div class="alert alert-danger">Pembayaran Order ID ${paymentResult.order_id} gagal. Pesan: ${paymentResult.status_message || 'Error'}.</div>`;
                        if (clickedButton) clickedButton.disabled = false;
                    },
                    onClose: function() {
                        ...
                        if (!snapMessageDiv.querySelector('.alert-success') && !snapMessageDiv.querySelector('.alert-warning') && !snapMessageDiv.querySelector('.alert-danger')) {
                            snapMessageDiv.innerHTML = '<div class="alert alert-secondary">Popup pembayaran ditutup.</div>';
                        }
                        if (clickedButton) clickedButton.disabled = false;
                    }
                });
            ...
            snapMessageDiv.innerHTML = `<div class="alert alert-danger">${error.message}</div>`;
            if (clickedButton) clickedButton.disabled = false;
        }
    }
</script>