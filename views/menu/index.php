...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.quantity-selector').forEach((selector, index) => {
        ...
        const minusBtn = selector.querySelector('.minus'); const plusBtn = selector.querySelector('.plus'); const input = selector.querySelector('.quantity-input'); const card = selector.closest('.menu-card');
        if (!minusBtn || !plusBtn || !input || !card) { console.warn(`Incomplete elements for selector #${selector.id}`); return; }
        const basePriceElement = card.querySelector('.base-price'); const totalPriceSpan = card.querySelector('.total-price');
        if (!basePriceElement || !totalPriceSpan) { console.warn(`Price elements not found for selector #${selector.id}`); return; }
        const basePrice = parseFloat(basePriceElement.value); const maxQty = parseInt(input.getAttribute('max') || '10');
        function updateDisplayAndValidate() { let currentValue = parseInt(input.value); if (isNaN(currentValue) || currentValue < 1) currentValue = 1; if (currentValue > maxQty) currentValue = maxQty; input.value = currentValue; const newTotal = basePrice * currentValue; totalPriceSpan.textContent = newTotal.toLocaleString('id-ID', { minimumFractionDigits: 0 }); }
        ...
        input.addEventListener('change', updateDisplayAndValidate); input.addEventListener('blur', updateDisplayAndValidate); updateDisplayAndValidate();
    });
    const orderModalElement = document.getElementById('orderModal');
    if (orderModalElement) {
        orderModalElement.addEventListener('hidden.bs.modal', function () {
            const orderForm = document.getElementById('orderForm'); if(orderForm) orderForm.reset();
            const paymentMsgDiv = document.getElementById('snap-payment-message'); if(paymentMsgDiv) paymentMsgDiv.innerHTML = '';
            const payButton = document.getElementById('payAndWhatsAppButton'); if(payButton) payButton.disabled = false;
        });
    }
});
</script>